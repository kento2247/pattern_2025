...
for k in range(K):
    for p in range(P):
        data[k * P + p][0], data[k * P + p][1] = np.random.multivariate_normal(
            average[k], cov[k]
        ).T
...
for loop in range(LOOP):
    ...
    # 平均値の更新
    new_average = np.zeros((K, D), dtype=np.float64)
    for k in range(K):
        sum_Q = 0
        for i in range(N):
            sum_Q += Q[i][k]
            new_average[k] += Q[i][k] * data[i]
        new_average[k] = new_average[k] / sum_Q
    e_average = new_average
    print(loop, ":average -> ", e_average)

    # 分散共分散行列の更新
    new_cov = np.zeros((K, D, D), dtype=np.float64)
    for k in range(K):
        sum_Q = 0
        for i in range(N):
            diff = np.reshape(data[i] - e_average[k], (D, 1))
            diff_T = np.reshape(data[i] - e_average[k], (1, D))
            new_cov[k] += Q[i][k] * np.dot(diff, diff_T)
            sum_Q += Q[i][k]
        new_cov[k] = new_cov[k] / sum_Q
    e_cov = new_cov
    print(loop, ":covariance -> ", e_cov)
    print(" ----- ")
...
for k in range(K):
    plt.figtext(
        (e_average[k][0] + 20) / 40, (e_average[k][1] + 20) / 40 / 2, plot_l[k], size=10
    )
